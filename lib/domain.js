  ...
  if (isActive) {
    ...
    statusMessage =
      now <= autoOpenEndsAt
        ...
        : powerOpen
          ? "تم فتح الطاقة باستثناء معتمد."
          : "انتهت نافذة التشغيل التلقائي. يلزم إدخال سبب لطلب فتح متأخر.";
  } else if (now > endAt) {
    phaseLabel = "منتهية";
    statusMessage = "انتهت الجلسة، ويمكن مراجعة السجل التشغيلي.";
  }
...
export async function getAdminOverview() {
  ...
  return {
    ...
    charts: {
      ...
      roomStateDistribution: Object.entries(stateCounts).map(([name, value]) => ({
        name,
        value,
      })),
    ...
    rooms,
    requests: withFacultyNames(db, [...db.requests].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt))),
    facultySummary,
...
function buildLogEntry(db, request, details, action, timestamp) {
  return {
    id: `log-${Date.now()}`,
...
export async function createFacultyRequest(payload) {
  ...
  return writeDb((db) => {
    ...
    const isAutoApproved = payload.type === "room_change" || payload.type === "extension";
    ...
    const nextRequest = {
      ...
      createdAt: timestamp,
      reviewedAt: isAutoApproved ? timestamp : null,
      reviewNote: isAutoApproved ? "اعتماد فوري للعرض التجريبي." : null,
    };

    db.requests.unshift(nextRequest);

    const roomName =
      nextRequest.requestedRoomId
    ...
    let message = "تم تسجيل الطلب وإحالته للمراجعة.";

    if (nextRequest.type === "late_open") {
      details = `${faculty.name} رفع طلب فتح متأخر في ${session.room.name} بسبب: ${nextRequest.reason}`;
      message = "تم رفع طلب الفتح المتأخر وإظهاره للإدارة للمراجعة.";
    }

    if (nextRequest.type === "room_change") {
      details = `${faculty.name} تم نقل محاضرته فورًا إلى ${roomName} بسبب: ${nextRequest.reason}`;
      message = `تم اعتماد نقل القاعة فورًا إلى ${roomName}.`;
    }

    if (nextRequest.type === "extension") {
      details = `${faculty.name} حصل على تمديد فوري لمدة ${nextRequest.requestedMinutes} دقيقة. السبب: ${nextRequest.reason}`;
      message = `تم اعتماد تمديد الجلسة ${nextRequest.requestedMinutes} دقيقة.`;
    }

    db.logs.unshift(buildLogEntry(db, nextRequest, details, action, timestamp));
...
export async function reviewRequest(requestId, decision, reviewNote) {
  ...
  return writeDb((db) => {
    ...
    db.logs.unshift(buildLogEntry(db, request, details, action, request.reviewedAt));
